...
def update(selected=None):
    ...
    selection_1 = np.array(source.data[~source.data["word"].isin(ss[0:int(stopwords_1.value)])]["freq_x"].astype(float))
    #selection_2 = np.array(load_ticker(t2)[~load_ticker(t2)["word"].isin(ss[0:int(stopwords_1.value)])]["freq"].astype(float))
    selection_2 = np.array(source.data[~source.data["word"].isin(ss[0:int(stopwords_1.value)])]["freq_y"].astype(float))
