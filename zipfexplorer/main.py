...
def update(selected=None):
    ...
    ', α = %s' % str(round(powerlaw.Fit(load_ticker(t1)[~load_ticker(t1)["word"].isin(ss[0:int(stopwords_1.value)])]["freq"].astype(float)).alpha,3)) +\
    ', H = %s' % str(round(ent(load_ticker(t1)[~load_ticker(t1)["word"].isin(ss[0:int(stopwords_1.value)])]["freq"].astype(float)),3))
